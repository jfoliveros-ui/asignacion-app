...
<head>
    <meta charset="UTF-8">
    <title>Actualización de solicitud de salón</title>
</head>
                            ...
                            <tr>
                                <td style="background:#f8fbff; border:1px solid #d7e5fb; border-radius:14px; padding:18px 20px;">
                                    ...
                                    <div style="font-size:15px; line-height:1.9; color:#1f2937; margin-top:12px;">
                                        ...
                                        <strong>Fecha solicitada:</strong> {{ $s->fecha ? \Carbon\Carbon::parse($s->fecha)->format('d/m/Y') : 'N/A' }}
                                    </div>
                                </td>
                            </tr>
                        </table>
                ...
                <tr>
                    <td style="padding:16px 40px 8px 40px;">
                        ...
                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse; border:1px solid #dbe4f0; border-radius:14px; overflow:hidden;">
                            ...
                            <tr>
                                <td style="background:#f8fbff; padding:14px 16px; border-bottom:1px solid #dbe4f0; font-weight:bold; color:#144A8A;">Correo institucional</td>
                                <td style="padding:14px 16px; border-bottom:1px solid #dbe4f0;">{{ $s->email ?? 'N/A' }}</td>
                            </tr>
                            <tr>
                                <td style="background:#f8fbff; padding:14px 16px; font-weight:bold; color:#144A8A;">Horario</td>
                                <td style="padding:14px 16px;">{{ substr((string) $s->hora_inicio, 0, 5) }} - {{ substr((string) $s->hora_fin, 0, 5) }}</td>
                            </tr>
                        </table>
                    </td>
                </tr>
                ...
                <tr>
                    <td style="padding:18px 40px 22px 40px;">
                        <div style="background:#eef4ff; border-left:5px solid #3366CC; border-radius:10px; padding:16px 18px; color:#344054; font-size:14px; line-height:1.7;">
                            Si requiere más información sobre esta actualización, por favor comuníquese con el área administrativa correspondiente.
                        </div>
                    </td>
                </tr>
